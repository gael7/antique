var management={
    ...
    $.getJSON("/products", function(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        // Display the apropos information on the page
          $("#productsTable").append("<tr><td>"+i+"</td><td>"+data[i].productName+"</td><td>"+data[i].productPrice+"</td><td>"+data[i].productCategory+"</td><td><a href='#' class='btn btn-info' id='updateProduct' data-id='"+data[i]._id+"' data-target='#updateProductModal' data-toggle='modal'>Update</a></td><td><button type='button' class='close' id='deleteProduct' data-id='"+data[i]._id+"'>&times;</button></td></tr>");
        }
    });
  },
  ...
  modalUpdateProduct: function(product){
    ...
    $(".modal-footer").append("<a href='#' class='btn btn-info col-lg-4' id='updateProductDb' data-id='"+product+"'>Update</a>");
